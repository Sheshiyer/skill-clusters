...
| Concern | A baseline formatter hook | Plankton |
|---------|---------------------------|----------|
| Code quality enforcement | PostToolUse hooks (Prettier, tsc) | PostToolUse hooks (20+ linters + subprocess fixes) |
| Security scanning | external security reviewer | Bandit (Python), Semgrep (TypeScript) |
...
1. Add Plankton hooks for write-time quality enforcement
2. Run a dedicated security audit on top
3. Use `verification-loop` as a final gate before PRs

### Avoiding Hook Conflicts

If running Plankton alongside a separate Prettier formatter hook:
- A standalone Prettier hook and Plankton's biome formatter may conflict on JS/TS files
- Resolution: disable the standalone Prettier PostToolUse hook when using Plankton (Plankton's biome is more comprehensive)
- Both can coexist on different file types (the standalone hook handles what Plankton doesn't cover)
...
```bash
export QUALITY_HOOK_PROFILE=strict
