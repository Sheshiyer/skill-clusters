...
This is the agent-native answer to setup-audit plugins. It does not modify files unless the user explicitly asks for follow-up implementation.
...
- Comparing official marketplace plugins against agent-native coverage
...
- Prefer agent-native workflows over generic "install another plugin" advice when the agent can reasonably own the surface.
...
- Separate three things clearly:
  ...
  - available but not wrapped well in the agent stack
...
2. Environment surface
   - `.env*` files in the active repo and obvious adjacent agent workspaces
...
- existing agent skills already relevant to the workspace

If a surface exists only as a primitive, call that out. Example:

- "Stripe is available via connected app, but the agent stack lacks a billing-operator skill"
- "Google Drive is connected, but there is no agent-native Google Workspace operator workflow"
...
2. whether the agent stack already has parity
3. whether the agent stack only has primitives
4. whether the agent stack is missing the workflow entirely

### Phase 3: Turn Gaps Into Agent-Stack Decisions

For every real gap, recommend the correct agent-native shape:

| Gap Type | Preferred Agent Shape |
|----------|---------------------|
...
2. **Parity**
   - where the agent stack already matches or exceeds the benchmark
3. **Primitive-only gaps**
   - tools exist, but the agent stack lacks a clean operator skill
4. **Missing integrations**
   - capability not available yet
5. **Top 3-5 next moves**
   - concrete agent-native additions, ordered by impact
...
- If the agent stack already has a strong primitive, propose a wrapper skill instead of inventing a brand-new subsystem.

## Good Outcomes

- The user can immediately see what is connected, what is missing, and what the agent stack should own next.
