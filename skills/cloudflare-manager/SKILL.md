...
1. **Install Dependencies**
   ```bash
   cd ~/.agents/skills/cloudflare-manager
   bun install
   ```
...
Agent: [Deploys worker using bun scripts/workers.ts deploy api-handler ./worker.js]
...
All scripts are located in `~/.agents/skills/cloudflare-manager/scripts/`:
...
Starter templates are available in `~/.agents/skills/cloudflare-manager/templates/`:
