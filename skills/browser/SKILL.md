...
```bash
# Open URL in user's preferred browser ($BROWSER env var, or system default)
...
bun run skills/browser/Tools/Browse.ts https://example.com
...
bun run Browse.ts open <url>                 # Open in user's preferred browser ($BROWSER or default)
```
...
**Location:** `skills/browser/Tools/Browse.ts`
...
| `open <url>` | Open in user's preferred browser ($BROWSER or default) |
| `status` | Session info |
...
**Location:** `skills/browser/Tools/BrowserSession.ts`
...
```typescript
import { PlaywrightBrowser } from 'skills/browser/index.ts'
