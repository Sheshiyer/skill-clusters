...
description: "Scoped CLAUDE.md memory system (HAM) that cuts context-token spend with directory-level context files, a decisions/patterns memory layer, and a savings dashboard. USE WHEN a multi-directory project keeps re-reading the same files and you want scoped agent memory."
