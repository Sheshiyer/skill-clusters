---
name: scroll-world
description: "Build an immersive scroll-scrubbed fly-through-the-world landing page for any brand or industry — cohesive AI scene stills + seamless camera clips (Monid Seedance / Higgsfield) wired to a portable vanilla-JS scroll engine. USE WHEN the user wants a 3D world, browse-through-the-industry hero, scroll cinematic, diorama landing, Emons-style continuous flight, or to turn a business into a scrollable world."
cluster: design
version: 1.0.0
origin: "oso95/scroll-world (MIT)"
---

# scroll-world

> **Cluster:** `design` · pairs with `brandmint` / `openbrand` for brand kit, `motionskin` for MotionSites template reskin (different path: this skill *generates* a scroll-cinematic world; motionskin *reskins* curated templates). Live in-browser GSAP scroll → creative-frontend cluster instead.
