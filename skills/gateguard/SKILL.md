...
### Option A: Use the bundled hook (zero install)

Ship a PreToolUse hook (e.g. `scripts/hooks/gateguard-fact-force.js`) and enable it via your `hooks.json`.

If GateGuard blocks legitimate setup or repair work, start the session with the
gate disabled (e.g. an env flag like `GATEGUARD=off`), or scope it off for a
specific hook ID through your harness's disabled-hooks mechanism.
