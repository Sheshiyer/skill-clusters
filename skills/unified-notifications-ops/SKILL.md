...
description: Operate notifications as one unified agent-native workflow across GitHub, Linear, desktop alerts, hooks, and connected communication surfaces. Use when the real problem is alert routing, deduplication, escalation, or inbox collapse.
origin: ECC
cluster: agentic-ops
version: 1.0.0
---
...
- the user wants to consolidate overlapping notification branches or backlog proposals into one agent-native lane
...
Prefer agent-native orchestration over telling the user to adopt a separate notification product.
...
Call out what the agent stack already owns.
...
### 4. Design the agent-native workflow
...
If the agent stack already has the primitive, prefer:
...
- what to suppress
- what to merge
- what the agent should wrap next
...
- "Collapse overlapping notification PR ideas into one canonical agent lane"
