...
description: "Analyze raw prompts, identify intent and gaps, match available agent components (skills/commands/agents/hooks), and output a ready-to-paste optimized prompt; advisory only. USE WHEN the user asks to optimize, improve, or rewrite a prompt; NOT FOR executing the task directly or optimizing code/performance."
origin: community
metadata:
  author: YannJY02
  version: "1.0.0"
...
Analyze a draft prompt, critique it, match it to the available agent
ecosystem components (skills, commands, agents, hooks), and output a complete
