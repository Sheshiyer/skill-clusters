---
name: recon
description: Security reconnaissance. USE WHEN recon, reconnaissance, bug bounty, attack surface, infrastructure mapping, subdomain enumeration.
cluster: security
version: 1.0.0
origin: PAI/Codex skills library
---
...
- `naabu` - Port scanning
- Note: Requires a security MCP profile providing these tools (enable per your MCP setup).
...
Reports saved to:
- **Work scratch/** - For iterative artifacts during investigation (a scratch dir under your working area)
- **Research archive** - For pentest engagements and formal assessments (a dated `research/YYYY-MM/` folder)

## MCP Profile Requirements

**For passive recon:** No special MCP profile needed

**For active recon:** Requires a security MCP profile. Enable the security MCP profile in your agent setup (the one providing `httpx`/`naabu`), restart the client to apply, then run active reconnaissance.
...
Assistant: Activating recon skill with passive-recon workflow.
     ...
     Report: scratch/recon-example-com/
```
...
**Security Skills:**
- `osint` - Entity and people reconnaissance
