...
description: Evidence-first automation inventory and overlap audit workflow for an autonomous agent operator. Use when the user wants to know which jobs, hooks, connectors, MCP servers, or wrappers are live, broken, redundant, or missing before fixing anything.
origin: ECC
cluster: agentic-ops
version: 1.0.0
---
...
Pull these agent-native skills into the workflow when relevant:
...
- `finance-billing-ops` when the real problem is webhook fanout, queued-job cost, or billing burn
...
- the user wants to know what was ported from another agent system and what still needs to be rebuilt inside the agent stack
...
The value is in collapsing noisy automation into one canonical agent lane, not in preserving every historical path.
