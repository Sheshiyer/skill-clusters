...
**Before executing, check for optional user customizations** in a sibling `USER/` override directory if your environment defines one. If present, load and apply a `PREFERENCES.md` (default genre, aesthetic, voice) plus any additional skill-specific files. Otherwise proceed with skill defaults.
