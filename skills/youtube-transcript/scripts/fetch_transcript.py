...
Originally designed to run via a residential IP (WireGuard VPN). If WireGuard tools
aren't available (e.g., on macOS without wg/wg-quick installed), set environment
variable YT_TRANSCRIPT_NO_VPN=1 to bypass VPN requirements.
"""
...
def check_vpn():
    ...
    If YT_TRANSCRIPT_NO_VPN=1, this returns True immediately.
    """
    if os.environ.get("YT_TRANSCRIPT_NO_VPN") == "1":
        return True, "VPN bypassed"
    try:
...
def fetch_transcript(video_id, languages=None):
    ...
    # Create session; optionally bind to VPN source IP
