...
type Model = "flux" | "nano-banana" | "nano-banana-pro" | "nano-banana-2" | "gpt-image-1" | "gpt-image-2" | "fal-flux" | "fal-flux-pro" | "fal-nano-banana-2";
...
Generate Unsupervised Learning branded images using Flux 1.1 Pro, Nano Banana, Nano Banana Pro, Nano Banana 2, or GPT-image-1/2.

USAGE:
  generate --model <model> --prompt "<prompt>" [OPTIONS]

REQUIRED:
  --model <model>      Model to use: flux, nano-banana, nano-banana-pro, nano-banana-2, gpt-image-1, gpt-image-2, fal-flux, fal-flux-pro, fal-nano-banana-2
...
--size <size>              Image size/aspect ratio (default: 16:9)
                              ...
                              OpenAI (gpt-image-1, gpt-image-2): 1024x1024, 1536x1024, 1024x1536
                              Gemini (nano-banana-pro, nano-banana-2, fal-nano-banana-2): 1K, 2K, 4K (resolution); aspect ratio inferred from context or defaults to 16:9
...
function parseArgs(argv: string[]): CLIArgs {
  ...
  for (let i = 0; i < args.length; i++) {
    ...
    switch (key) {
      case "model":
        if (value !== "flux" && value !== "nano-banana" && value !== "nano-banana-pro" && value !== "nano-banana-2" && value !== "gpt-image-1" && value !== "gpt-image-2" && value !== "fal-flux" && value !== "fal-flux-pro" && value !== "fal-nano-banana-2") {
          throw new CLIError(`Invalid model: ${value}. Must be: flux, nano-banana, nano-banana-pro, nano-banana-2, gpt-image-1, gpt-image-2, fal-flux, fal-flux-pro, or fal-nano-banana-2`);
        }
        parsed.model = value;
  ...
  if (parsed.model === "gpt-image-1" || parsed.model === "gpt-image-2") {
  ...
  } else if (parsed.model === "nano-banana-pro" || parsed.model === "fal-nano-banana-2" || parsed.model === "nano-banana-2") {
...
async function generateWithFal(prompt: string, size: Size, output: string, model: "fal-flux" | "fal-flux-pro"): Promise<void> {
  const apiKey = process.env.FAL_KEY || process.env.FAL_API_KEY;
  if (!apiKey) {
    throw new CLIError("Missing environment variable: FAL_KEY");
  }

  const falModel = model === "fal-flux-pro" ? "fal-ai/flux-pro/v1.1" : "fal-ai/flux/dev";
  console.log(`Generating with ${model} (${falModel})...`);

  // Map aspect ratios to FAL format
  let image_size: string | object = size;
  if (size === "16:9") image_size = "landscape_16_9";
  else if (size === "1:1") image_size = "square";
  else if (size === "4:3") image_size = "landscape_4_3";
  else if (size === "9:16") image_size = "portrait_16_9";
  else if (size === "3:4") image_size = "portrait_4_3";

  // Use fal.run for synchronous results
  ...
  if (!response.ok) {
    const errorText = await response.text();
    throw new CLIError(`FAL.AI API error: ${response.status} - ${errorText}`);
  }
  ...
  if (!imageUrl) {
    throw new CLIError("No image URL returned from FAL.AI API");
  }
...
async function generateWithNanoBanana2Fal(
  prompt: string,
  resolution: "1K" | "2K" | "4K",
  aspectRatio: string,
  output: string
  ...
  console.log(`Generating with Nano Banana 2 via FAL.ai (${resolution}, ${aspectRatio})...`);

  // Map aspect ratios to FAL format for nano-banana-2
  ...
  const falAspectRatio = aspectRatioMap[aspectRatio] || "1:1";
...
async function generateWithGPTImage2(prompt: string, size: OpenAISize, output: string): Promise<void> {
  const apiKey = process.env.FAL_KEY || process.env.FAL_API_KEY;
  if (!apiKey) {
    throw new CLIError("Missing environment variable: FAL_KEY");
  }

  console.log("Generating with GPT Image 2 via FAL.ai...");

  // Map OpenAI size format to FAL gpt-image-2 size format
  ...
  const falSize = sizeMap[size] || "1024x1024";
  ...
  if (!response.ok) {
    const errorText = await response.text();
    throw new CLIError(`FAL.AI API error (gpt-image-2): ${response.status} - ${errorText}`);
  }
  ...
  if (!imageUrl) {
    throw new CLIError("No image URL returned from FAL.AI gpt-image-2 API");
  }
  ...
  try {
    ...
    if (args.creativeVariations && args.creativeVariations > 1) {
      ...
      for (let i = 1; i <= args.creativeVariations; i++) {
        ...
        if (args.model === "flux") {
        ...
        } else if (args.model === "fal-flux" || args.model === "fal-flux-pro") {
          promises.push(generateWithFal(finalPrompt, args.size, varOutput, args.model));
        } else if (args.model === "fal-nano-banana-2") {
          promises.push(generateWithNanoBanana2Fal(finalPrompt, args.size as GeminiSize, args.aspectRatio!, varOutput));
          ...
          promises.push(generateWithGPTImage2(finalPrompt, args.size as OpenAISize, varOutput));
        }
      }
    ...
    if (args.model === "flux") {
      ...
      await generateWithFal(finalPrompt, args.size, args.output, args.model);
    } else if (args.model === "fal-nano-banana-2") {
      await generateWithNanoBanana2Fal(finalPrompt, args.size as GeminiSize, args.aspectRatio!, args.output);
    } else if (args.model === "nano-banana") {
      ...
      await generateWithGPTImage2(finalPrompt, args.size as OpenAISize, args.output);
    }
